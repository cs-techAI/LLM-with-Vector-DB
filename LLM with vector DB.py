...
# Try to import sentence_transformers; if it fails, set a flag
try:
    from sentence_transformers import SentenceTransformer
    use_embeddings = True
except ImportError:
    st.warning("sentence-transformers library not found. Embeddings will not be generated.")
...
# Load the Sentence Transformer model for embeddings (if available)
@st.cache_resource
...
if st.button("Process"):
    ...
    # Generate embeddings for the content (if available)
    if use_embeddings:
        embedding_model = load_embedding_model()
        embeddings = embedding_model.encode([file_content])

        # Display embedding information (optional)
